...
pub fn startup_log_lines() -> Vec<String> {
    vec![
        ...
        "当前版本：v0.21.0-alpha.1，Python 报告辅助 API 示例、递归插件扫描、结构化 Python 自动化报告、headless 搜索报告、伪代码/IR headless 导出、伪代码/IR 搜索、正式 headless analyze 入口、本地签名库、Runtime 识别、运行库过滤和基础 x64 伪 C/IR 已接入。".to_owned(),
