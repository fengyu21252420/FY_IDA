...
FY_IDA v0.31.0-alpha.1 and later expose a lightweight script API through environment variables, the headless JSON report model, and a JSON action file for saved annotations.
...
for symbol in project.pdb_symbols("main"):
    print(symbol.kind, symbol.name, symbol.get("source"))
